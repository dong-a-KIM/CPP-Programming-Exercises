#include "MonsterWorld9_2.h"
#include "VariousMonsters9_2.h"
#include <time.h>
...
int main()
{
	...
	game.add(new Smombi("player1", "○", rand() % w, rand() % h));
	game.add(new Siangshi("player2", "▲", rand() % w, rand() % h));
	game.add(new MyMonster("player3", "♠", rand() % w, rand() % h));
	game.play(500, 10);
	printf("------게임 종료-------------------\n");
};
